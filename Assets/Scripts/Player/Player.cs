    ...
    public class Player : Entity
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (_isHorizontalDashing)
            {
                ...
                _rb.AddForce(new Vector2(_dashMultiplier, _rb.linearVelocity.y), ForceMode2D.Impulse);
            }

            if (_isVerticalDashing)
            {
                _isVerticalDashing = false;
                _rb.AddForce(new Vector2(_rb.linearVelocity.x, _dashMultiplier), ForceMode2D.Impulse);
            }
        }
